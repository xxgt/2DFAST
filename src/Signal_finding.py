...
def Signal_finding(DM_axis,DM_axis_cuda,threshold,data,pixel,DM_range=[50,2000],seq=0,In_snr=0):
	'''
	...
	# a_i, b_i =np.histogram( d_i,bins=nbin,density=True)
	...
	# print(data_mean, mu_c)
	# std_c= abs(a_r_std  + a_i_std*1j)
	# print(std_c, sig, a_r_std, a_i_std, r_std, i_std, sig/2**0.5)
	std_c= sig / 2 ** 0.5
	data = (data-data_mean) / (std_c)
	data = (abs(data))**2
#######
#Test##
######
...
#	 DM_list = data.sum(axis=0)
	...
	lo_threshold = cp.where(data > cut_off)

	n_count , _ = np.histogram(DM_axis[lo_threshold[1].get()],bins=DM_axis.size,range=(DM_axis.min(),DM_axis.max()))

	data = data - cut_off
	data = cp.clip(data,0,data.max())
	# time_gpu = time.time() - time_s
	DM_list = data.sum(axis=0).get()
	# time_io	= time.time() - time_s - time_gpu
#	 test_SHOW =1
...
#                plt.axvline(x=100,color='red',linestyle='--')
	...
	lo_threshold = lo_threshold[0]
	for ii in range(len(lo_threshold)):
		for jj in range(len(lo_threshold)):
			if abs(lo_threshold[ii]-lo_threshold[jj]) < pixel:# and ii!=jj:
				n_count1[lo_threshold[ii]] += n_count[lo_threshold[jj]]
				DM_list1[lo_threshold[ii]] += DM_list[lo_threshold[jj]]
#  	 DM_list1 += DM_list
#	 n_count1 += n_count
#         if seq == 100 and test_SHOW==1:
#                 plt.subplot(2,2,3)
	...
	if len(SNR) > 0:
		...
		if n_count1.max() >=2:
			...
			if len(lo_intersection) != 0:
				l = int(len(lo_intersection)/2-1)				
				dm =  DM_axis[lo_intersection[l]]
				points = n_count1[lo_intersection]
				significance= ((DM_l.max() - data.mean())/sig/points[l]).get()
				print("-------------------------------")
...
#				 Gtransient.extend([dm,seq,alfa])
	# time_cpu = time.time() -time_s - time_gpu - time_io
	# print(time_gpu, time_io, time_cpu)
