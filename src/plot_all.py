...
def plot(t_axis , data_cuda , re_data , polar_data , FFT1st_data , FFT2nd_data,\
	...
	Imag_show = 6
	if   process  == '':
		if SHOW ==1:    print('Please claim which process to plot if you want to make plot!')
	if 'raw' in process:
		data = cp.asnumpy(data_cuda)
		data  = np.ma.masked_invalid(np.abs(data))
		mean  = np.mean(data)
		sigma = np.var(data)
		t_x   = t_axis[len(t_axis) - data.shape[0]:]
		print(len(t_x))
		data = data.transpose()
		max   = mean + 2 * sigma
		min   = mean - 2 * sigma
		...
		if seq == Imag_show and SHOW == 1:
			plt.show()
		plt.close()
		print('raw data plot is over...')

	if 'rebin' in process:
		re_data_cpu = cp.asnumpy(re_data)
		data  = np.abs(re_data_cpu)
		mean  = np.mean(data)
		sigma = np.var(data)
		max   = mean + 2 * sigma
		min   = mean - 2 * sigma
		...
		if seq == Imag_show and SHOW == 1:
			plt.show()
		plt.close()
		print('rebin plot over...')

	if '1stFFT' in process:
		FFT1st_data_cpu = cp.asnumpy(FFT1st_data)
		data  = np.abs(FFT1st_data_cpu)
		mean  = np.mean(data)
		sigma = np.var(data)
	#   max   = mean + 2 * sigma
	#   min   = mean - 2 * sigma
...
#		data[-3:,:4]=data.mean()
	#	data[:,0]=data.mean()
		...
		plt.ylabel('Frequency after 1st FFT')
		plt.colorbar()
...
#       np.savetxt('/home/nch/cord.txt',cord)
	...
	#	arrowprops = dict(facecolor = 'red', shrink = 0.01))
		...
		if seq == Imag_show and SHOW==1:
			plt.show()	
		plt.close()
		print('1stFFT plot over....')

	if 'polar_sets_3D' in process:
		polar_data_cpu = cp.asnumpy(polar_data)
		data  = np.abs(polar_data_cpu)
#       mean  = np.mean(data)
#       sigma = np.var(data)
#       max   = mean + 2 * sigma
#       min   = mean - 2 * sigma
...
#       plt.title('polar Sum along radius axis(grid size:'+ str(r_r)+'*'+str(a_r)+')')
		...
		if seq == Imag_show and SHOW==1:
			plt.show()
		plt.close()
		print('polar_2D plot over...')

	if '2ndFFT_3D' in process:
		data  = np.abs(FFT2nd_data.get())
#       mean  = np.mean(data)
#       sigma = np.var(data)
#       max   = mean + 2 * sigma
#       min   = mean - 2 * sigma
...
#               data[ind] = 0
		...
		plt.close()
		#exit(1)
		print('2nd FFT 3D plot over...')

	if '2ndFFT_2D' in process:
		data  = FFT2nd_data.get()
		data  = abs(data)
		data  = data.sum(axis = 0)
		seq   = i_ch
		#Filter the profile of the FRB signal
