...
#import pyfits
#import h5py
def read_data(f_name, t_len, nbin, DM_range=[100,1500],Wp=10,ang=[0,0]):
	'''
...
#	C	= 4.148908e6 # (ms)
	...
	t_gulp = int(2**(round(np.log2(t_lenr))))
...
#		t_len  = t_lenr
	...
	# print("time length: ", t_len, ", time unit: ", t_rsl)
	freq = np.linspace(ftop,fbot,nch)
	fy = freq**-2

	fy_cuda = cp.array(fy).astype(np.float32)
	f_cuda = cp.linspace(fy.min(),fy.max(),nbin).astype(np.float32)
	idx_cuda = cp.searchsorted(fy_cuda, f_cuda, 'left').astype(np.int32)

	# chunks
	num = int(nsamp/t_len)	#Total number of chunks to process 
	p_n = num	#Number of chunks for each thread to process
...
#Image parameter
	FFT_rs = ((1./nbin)**2 + (1./(2*t_len))**2)**0.5 #For pixel
	angle	= angle_range(fy,DM_range,nbin,2*T)
...
#		print 'Using matrix angle resolution..'
	...
	DM_axis = np.linspace(DM_range[0],DM_range[1],n_deg)
	DM_axis_cuda = cp.linspace(DM_range[0],DM_range[1],n_deg)
...
# Radius resolutiongrid size for interpolate in polar-coordin transform.
	...
	return fil, num, p_n, freq, t_rsl, t_len, t_gulp, nbin, nch, T, fy, fy_cuda, f_cuda, idx_cuda, angle, n_deg, L_fft, DM_axis, DM_axis_cuda

if   __name__ == '__main__':
	f_dir  = '../data/'
	f_name = 'data_2017-08-30_17-35-36.fil'
	f, num, p_n, freq, t_rsl, t_len, _ = read_data(f_dir ,f_name ,0,0,10)
	print(freq.shape)
	print(p_n)
	b = f.read_block(0,100)
	print(b.shape)
	print('load over!')
	exit()
