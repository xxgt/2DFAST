...
def FFT(data, Dim  ,L_fft = 0, msk_cycle = 0,t_gulp =0,W_f=1):
	...
	#	   L_fft  = min(data.shape)
		# if data.shape[-1] < 2 * L_fft:
		# 	data = cp.fft.fft2(data, norm="ortho")
		# else:
		data   = cp.fft.rfft2(data,norm = 'ortho')
		data	  = data[int(-L_fft):, 1:int(L_fft)+1]
		
		blackman_x    = cp.blackman(data.shape[1]*2)
		blackman_y    = cp.blackman(data.shape[0]*2)
		#Window function
	...
	#	data   = np.pad(data,[(pad_l,0),(0,0)],mode='constant')
		data   = cp.fft.fft(data,axis=0,norm = 'ortho')
		data   = cp.fft.fftshift(data,axes=0)
