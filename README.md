# 2DFAST
--This is the GPU-version of the pipeline using 2DFFT method to search Fast Radio burst  
...
Run the `__main__.py`in the `src` directory

	python3 __main__.py -h:

	Usage: python __main__.py  [options] 
	...
	--nthreads=NTHREADS   num of IO threads
	...
	--Plot_proc=PLOT_PROC
							...
							polar_sets_3D,                     polar_sets_2D,
							2ndFFT_3D, 2ndFFT_2D}  (This function Remain updates)
 
== Requires `filterbank (.fil)` file as input file. `Readers.py` From SIGPYPROC is used as one of our dependent package. ==
